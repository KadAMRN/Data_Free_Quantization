...
# Custom addition operation supporting quantization.
# It replaces the standard addition operation in neural networks to incorporate quantization.
def ___add__(input, *args):
    global raw_tensor_magic_op, module_tensor_op
    _stack = inspect.stack()
    # Check if the operation is called from within a forward pass and is the target of quantization.
    if 'forward' == _stack[1].function and '{}_{}_2'.format('add', _stack[1].lineno) == module_tensor_op.get_module_name():
        input = module_tensor_op(input)  # Apply quantization to the input tensor.
        args = [module_tensor_op(args[0])]  # Apply quantization to additional arguments.

        module_tensor_op.add_idx_name_tensor_op()  # Update the quantization operation index.

    x = raw_tensor_magic_op['__add__'](input, *args)  # Perform the actual addition operation.

    del _stack

    return x

# Similar to ___add__, this is a custom in-place addition operation for quantized tensors.
...
# Wrapper for PyTorch's cat (concatenate) function to include quantization support.
...
# Custom implementation of the mean calculation to support quantization.
...
# Custom interpolation function that integrates quantization.
...
# Custom softmax function with quantization support.
...
# Function to replace standard PyTorch operations with custom quantized operations.
def replace_op():
...
# Restores the original PyTorch operations, undoing the effect of replace_op.
def restore_op():
    global tensor_magic_op_supported, raw_tensor_magic_op, torch_op_supported, raw_torch_op, func_op_sopprted, raw_func_op
...
# Main function to switch layers in a model to their quantized counterparts.
...
# Custom module for applying a sequence of tensor operations, typically used for quantization.
...
# Merges BatchNorm parameters into convolutional layers as part of the quantization process.
...
# Applies quantization to the parameters (weights and biases) of specific layers.
...
# Helper function to locate BatchNorm layers and determine their relationships in the network.
